...
use contract::{ErrorCode, MaintenanceInstallApk, MaintenanceInstallModule, MaintenanceStatus, UuidV4};
...
impl MaintenanceAttempts {
    ...
    fn try_install(
        ...
        if !target.matches || target.size == 0 || target.size > MAX_ARTIFACT_BYTES {
            return Err(DomainError::invalid("maintenance request does not match the signed record"));
        }
        ...
        let (program, arguments) = match kind {
            InstallKind::Apk => (PM, vec!["install".into(), "-r".into(), "--user".into(), "0".into(), staged_path]),
            InstallKind::Module => (MAGISK, vec!["--install-module".into(), staged_path]),
        };
        let settled = self
            .claims
            .claim(target.execution_id.clone())
            .map_err(|error| runtime::ExecutionFailure { error, cleanup_verified: true })
            .and_then(|claim| guard.run_maintenance_install(&target.execution_id, program, arguments, &claim));
...
fn stage(artifact: OwnedFd, target: &Target) -> Result<PathBuf, DomainError> {
    ...
    let metadata = source.metadata().map_err(|_| io_error("cannot inspect maintenance artifact"))?;
    if !metadata.is_file() || metadata.len() != target.size {
        return Err(DomainError::invalid("maintenance artifact is not the signed regular file"));
    }
    ...
    let copied = (|| {
        ...
        let mut total = 0_u64;
        loop {
            let read = source.read(&mut buffer).map_err(|_| io_error("cannot read maintenance artifact"))?;
            if read == 0 {
                break;
            }
            total += read as u64;
            if total > target.size {
                return Err(DomainError::invalid("maintenance artifact exceeds its signed size"));
            }
            digest.update(&buffer[..read]);
            staged.write_all(&buffer[..read]).map_err(|_| io_error("cannot stage maintenance artifact"))?;
        }
        staged.sync_all().map_err(|_| io_error("cannot sync maintenance artifact"))?;
        let actual: String = digest.finalize().iter().map(|byte| format!("{byte:02x}")).collect();
        if total != target.size || actual != target.sha256 {
            return Err(DomainError::invalid("maintenance artifact does not match its signed digest"));
        }
        Ok(())
    })();
...
fn read_record(base: &Path) -> Result<MaintenanceRecord, DomainError> {
    ...
    let record: MaintenanceRecord =
        serde_json::from_slice(&bytes).map_err(|_| io_error("update maintenance record is malformed"))?;
...
fn decode<T: serde::de::DeserializeOwned>(payload: &Value) -> Result<T, DomainError> {
    serde_json::from_value(payload.clone()).map_err(|_| DomainError::invalid("invalid maintenance payload"))
}
